...
userinput=str(input('ENTER THE LOCATION : '))
foldername=str(input('ENTER NAME OF DESTINATION FOLDER : '))
sr=userinput.replace('\\','/')
...
for y in f_list :
    ...
    if len(x)==2 :
        ...
        pth=extension_paths['.'+x[1]]
        newpth=src +'/' + pth
    
        if not os.path.exists(newpth) :
            os.makedirs(newpth)
        mainpth=newpth + '/' + x[0] +'.' + x[1]
            
        i=0
        while os.path.exists(mainpth):
            i=i+1
            y=str(i)
            mainpth=newpth + '/' + x[0] + y +'.' + x[1]
        if i != 0 :
            roldname=sr+'/'+x[0]+'.'+x[1]
            rnewname=sr+'/'+x[0]+y+'.'+x[1]
            os.rename(src=roldname,dst=rnewname)
            x[0]=x[0]+y

        
        mov=sr + '/' + x[0] +'.' + x[1]
        shutil.move(src=mov ,dst=newpth)
